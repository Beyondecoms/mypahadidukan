...
document.addEventListener('click', function(e) {
  const btn = e.target.closest('.m-product-card__quick-add-btn');
  if (!btn) return;
  const card = btn.closest('.m-product-card');
  const qtyInput = card && card.querySelector('.m-product-card__qty-input');
  ...
  fetch('/cart/add.js', {
    ...
    // Refresh cart drawer or mini cart here if needed.
  });
});
